...
###### ARGUMENT PARSING ######
suppressWarnings(suppressPackageStartupMessages(require(optparse))) # don't say "Loading required package: optparse"
...
suppressWarnings(suppressPackageStartupMessages(library(fpc))) # clusterboot function
...
###### ADAPTED FUNCTIONS #####
 ...
 ANI.dendrogram.custom <- function (ani_object, ANI_threshold = 95,
   ...
   bs.results <<- clusterboot(d.dist, B=bootstrap_iterations,
                       distances=TRUE,
                       k=nclus,
                       multipleboot=TRUE, #TRyING THIS OUT
                       bootmethod = "boot",
 ...
 plot.dendrogram.custom <- function(xline = NULL, xlinecol = "#20A387FF", 
   ...
   for (name in names(cluster_ids)) {
     ...
     text((xy[current_node_xy,][2]+0.5),# + xy[closest_node_xy,][2] / 2),
          ...
          labels=bsvalue,
          cex=0.7, font=2,
          col=ifelse(bsvalue >= 75,"blue","red"))
   }
