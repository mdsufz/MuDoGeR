...
In this special step, the user can pick representative Prokaryotic Metagenome-Assembled Genomes.
Running 2.b:
```
mudoger 2.b -i ~/path/to/gtdb_taxonomy/file -b ~/path/to/bbtools/file -m ~/path/to/bins(mags)/folder -o ~/path/to/output/folder
``` 
* The `/path/to/gtdb_taxonomy/file` indicates the path to the taxonomy file, generated by **GTDB-Tk**.
* The `/path/to/bbtools/file` indicates the path to the file with bbtools.
* The `/path/to/bins(mags)/folder`  indicates the path to the the bins(mags) folder.
