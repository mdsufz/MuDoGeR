...
The **MuDoGeR** Raw Read Quality Control is based on **metaWrap** (Uritskiy et al., 2018) Read Quality Control and is responsible to prepare the raw reads for assembly and alignment. The trimming of the raw reads is based on adapted content and **PHRED** scored with the default setting of Trim-galore (Krueger, 2015). As a result, all the non-high quality sequences are removed. The high quality reads that remained are aligned to the genome of a potential host with bmtagger (Agarwala and Morgulis, 2010), a process that leads to the removal of host contamination and of read pairs with only one aligned read from the metagenomic data. In the final step, **FASTQC** (Andrews, 2010) is used to assess read quality improvement.
